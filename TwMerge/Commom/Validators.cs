...
namespace TwMerge.Commom {
    ...
    public static partial class Validators {
      ...
      => !string.IsNullOrEmpty(value) && double.TryParse(value, out _);
        ...
        private static readonly Regex ArbitraryValueRegex = new Regex(@"^\[(?:(\w[\w-]*):)?(.+)\]$", RegexOptions.IgnoreCase);
        private static readonly Regex ArbitraryVariableRegex = new Regex(@"^\((?:(\w[\w-]*):)?(.+)\)$", RegexOptions.IgnoreCase);
        ...
        private static readonly Regex LengthUnitRegex = new Regex(@"\d+(%|px|r?em|[sdl]?v([hwib]|min|max)|pt|pc|in|cm|mm|cap|ch|ex|r?lh|cq(w|h|i|b|min|max))|\b(calc|min|max|clamp)\(.+\)|^0$", RegexOptions.IgnoreCase);

        private static readonly Regex ColorFunctionRegex = new Regex(@"^(rgba?|hsla?|hwb|(ok)?(lab|lch))\(.+\)$", RegexOptions.IgnoreCase);

        private static readonly Regex ShadowRegex = new Regex(@"^(inset_)?-?((\d+)?\.?(\d+)[a-z]+|0)_-?((\d+)?\.?(\d+)[a-z]+|0)", RegexOptions.IgnoreCase);

        private static readonly Regex ImageRegex = new Regex(@"^(url|image|image-set|cross-fade|element|(repeating-)?(linear|radial|conic)-gradient)\(.+\)$", RegexOptions.IgnoreCase);
